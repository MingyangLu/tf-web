  ...
  <div class="order_block">
    ...
    >
      ...
      <el-form-item label="Target score：" prop="target">
        <tf-select
          v-model="orderForm.target"
          option="target"
          placeholder="目标分数"
        ></tf-select>
      </el-form-item>
      <el-form-item label="Due date:" prop="deadline">
        <el-date-picker
      ...
      <el-form-item class="button-block">
        <el-button type="primary" @click="createOrder('orderForm')"
          >立即创建</el-button
        >
  ...
  data() {
    return {
      ...
      rules: {
        ...
        grade: [{ required: true, message: "请选择写作水平", trigger: "blur" }],
        target: [
  ...
  methods: {
    ...
    async createOrder(formName) {
      await this.$refs[formName].validate((valid) => {
        if (valid) {
          let day = this.$utils.date.diffDays(
            this.orderForm.deadline,
            new Date()
          );
          if (day <= 7) {
          ...
          this.$http
            ...
            .then((res1) => {
              ...
              if (res1.data.resposeCode === "10001") {
                ...
                this.$alert(
                  "亲，由于系统原因下单失败，您可直接联系工作人员下单，微信号：catAshera3",
                  "",
                  {
                    confirmButtonText: "确定",
                  }
